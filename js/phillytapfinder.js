...
SearchPTF.prototype.getData = function(searchValue){
	...
	this.xhr = $.ajax({
		type: "POST",
		url: "/wp-content/plugins/phillytapfinder/",
		data:
		"class=Search" +
		"&process=searchAll" +
...
SearchPTF.prototype.showResults = function(){
	...
	if(that.data.beers.length > 0){
		$('#search-tap-tmpl').tmpl(that.data.beers).insertAfter('#search-tap-list .auto-list-title');
		this.tapList.show();
	}else{
		this.tapList.hide(0);
	}
	...
	if(that.data.styles.length > 0){
		$('#search-style-tmpl').tmpl(that.data.styles).insertAfter('#search-style-list .auto-list-title');
		this.styleList.show();
	}else{
	...
	if(that.data.bars.length > 0){
		$('#search-bar-tmpl').tmpl(that.data.bars).insertAfter('#search-bar-list .auto-list-title');
		this.barList.show();
	}else{
		this.barList.hide(0);
	}
	...
	if(that.data.hoods.length > 0){
		$('#search-hood-tmpl').tmpl(that.data.hoods).insertAfter('#search-hood-list .auto-list-title');
		this.hoodList.show();
	}else{
	...
	if(that.data.hoods.length == 0 && that.data.bars.length == 0 && that.data.styles.length == 0 && that.data.beers.length == 0){
		$('#search-hood-tmpl').tmpl(that.data.hoods).insertAfter('#search-hood-list .auto-list-title');
		this.noResults.show();
	}else{
...
PTFAds.prototype.showLeaderboard = function() {
	...
	$('#ad-leaderboard-tmpl').tmpl(that.data).appendTo('.ad-leaderboard');
}
...
PTFAds.prototype.showSingleSquareAd = function() {
	...
	console.log(that.data);
	window.ga('send', 'event', 'Ad', 'Viewed', decodeHtml(that.data.ad_title));
	$('#single-square-tmpl').tmpl(that.data).appendTo('.ad');
}
...
PTFAds.prototype.show3Ads = function() {
	...
	$('#three-square-tmpl').tmpl(that.data).prependTo('.ads');
}
...
$(document).ready(function(){
	...
	if ( $('body').hasClass('tax-ptf_beer_style') ) {
		singleSquareAd.getSingleSquareAd('tax-ptf_beer_style', 'ptf_beer_style_'+currID);
	}
